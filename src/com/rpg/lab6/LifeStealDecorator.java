...
public class LifeStealDecorator extends AttackDecorator{
    ...
    public LifeStealDecorator(Attack wrappedAttack, double ratio) {
        super(wrappedAttack);
        if (ratio < 0 || ratio > 1) throw new IllegalArgumentException("Life steal ratio must be between 0 and 1.");
        this.ratio = ratio;
    }
    ...
    public void attack(Character attacker, Destructible target) {
        ...
        wrappedAttack.attack(attacker, target); // ตีก่อน ค่อยดูดเลือด
